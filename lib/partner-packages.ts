...
const fields = "id,partner_id,name,description,resource_key,duration_minutes,buffer_before_minutes,buffer_after_minutes,price_amount,currency,included_items,is_published";

function mapPackage(row: Record<string, unknown>): PartnerPackage {
  return {
    id: String(row.id), partnerId: String(row.partner_id), name: String(row.name), description: String(row.description ?? ""),
...
function payload(input: PartnerPackageInput) {
  return {
    name: input.name.trim(), description: input.description.trim() || null, resource_key: input.resourceKey.trim().toLowerCase(),
