...
function CommunicationHub({demoMode,onSchedule}:{demoMode:boolean;onSchedule:(startsAt:string)=>void}) {
  ...
  const [messages,setMessages]=useState(demoMode?[{id:"welcome",from:"Serdar & Seda",text:"Hallo, wir freuen uns sehr auf die Reportage. Können wir kurz über unseren Ablauf sprechen?",time:"Heute · 10:24"}]:[]);
  const [draft,setDraft]=useState(""); const [callDate,setCallDate]=useState("2027-06-21"); const [callTime,setCallTime]=useState("18:30"); const [notice,setNotice]=useState("");
  const send=()=>{const text=draft.trim();if(!text)return;setMessages(current=>[...current,{id:`message-${Date.now()}`,from:"Ihr",text,time:"Gerade eben"}]);setDraft("");};
  const schedule=()=>{if(!callDate||!callTime)return;const startsAt=new Date(`${callDate}T${callTime}:00`).toISOString();onSchedule(startsAt);setNotice(`Videocall für ${new Date(startsAt).toLocaleDateString("de-DE",{day:"2-digit",month:"long"})} um ${callTime} Uhr vorgemerkt und im Kalender eingetragen.`);};
  return <section className={c.communication}><header><div><small>Direkt nach der Buchung</small><h1>Nachrichten & Gespräche</h1><p>Klärt Details direkt mit euren Paaren – ohne den Buchungsprozess in eine Anfrage zu verwandeln.</p></div><span>{demoMode?"Demo-Unterhaltung":"Nach der ersten Buchung aktiv"}</span></header>{demoMode?<div className={c.communicationGrid}><article className={c.chatCard}><div className={c.chatHead}><div><b>Serdar & Seda</b><small>Hochzeit · 19. Juni 2027</small></div><span>● Online</span></div><div className={c.messages}>{messages.map(message=><div className={message.from==="Ihr"?c.ownMessage:c.customerMessage} key={message.id}><p>{message.text}</p><small>{message.time}</small></div>)}</div><div className={c.composer}><input aria-label="Nachricht an Serdar und Seda" onChange={event=>setDraft(event.target.value)} onKeyDown={event=>{if(event.key==="Enter"){event.preventDefault();send();}}} placeholder="Nachricht schreiben …" value={draft}/><button onClick={send} type="button">Senden</button></div></article><aside className={c.videoCard}><small>Persönlich abstimmen</small><h2>Videocall vorschlagen</h2><p>Ein kurzer Termin für Ablauf, Wünsche und offene Fragen.</p><label>Datum<input onChange={event=>setCallDate(event.target.value)} type="date" value={callDate}/></label><label>Uhrzeit<input onChange={event=>setCallTime(event.target.value)} type="time" value={callTime}/></label><button onClick={schedule} type="button">Videocall vorschlagen →</button>{notice&&<p role="status">{notice}</p>}<em>Der Termin erscheint vorläufig im Ouivio-Kalender. Der Videolink wird im Live-Betrieb erst nach Zusage erzeugt.</em></aside></div>:<article className={c.communicationEmpty}><span>◌</span><h2>Noch keine Kundennachrichten</h2><p>Nach einer Direktbuchung wird die Unterhaltung automatisch mit dem jeweiligen Paar verknüpft. So bleiben Buchung und Kommunikation sauber getrennt.</p></article>}</section>;
}
