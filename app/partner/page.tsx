...
function CalendarConnections() {
  ...
  return <article className={styles.connections}><div><small>Schnittstellen</small><h2>Kalender verbinden</h2><p>Prüft einen Zugang zunächst sicher. Erst nach der Freigabe werden belegte Zeiten automatisch abgeglichen.</p></div>
    ...
    {appleOpen && <form className={styles.appleForm} onSubmit={testAppleConnection}><header><span>1</span><div><strong>Apple Calendar verbinden</strong><p>Für iCloud Calendar benötigt Ouivio ein separates Apple-App-Passwort – niemals euer normales Apple-ID-Passwort.</p></div></header><ol><li>Öffnet eure Apple-Account-Verwaltung.</li><li>Erstellt unter <b>„Anmelden & Sicherheit“</b> ein <b>App-spezifisches Passwort</b>.</li><li>Gebt ihm den Namen <b>„Ouivio Kalender“</b> und kopiert das erzeugte Passwort hierher.</li></ol><a className={styles.appleHelpLink} href="https://account.apple.com" target="_blank" rel="noreferrer">Apple-App-Passwort erstellen <span aria-hidden="true">↗</span></a><aside><b>Wichtig</b><span>Das App-Passwort ist ein individuell von Apple erzeugter Zugang nur für Ouivio. Ihr könnt es jederzeit in eurem Apple-Account widerrufen.</span></aside><div className={styles.appleFields}><label>Apple-ID<input aria-invalid={Boolean(appleError)} autoComplete="username" type="email" value={appleId} onChange={(event) => setAppleId(event.target.value)} placeholder="name@icloud.com" required /></label><label>App-spezifisches Passwort<input aria-invalid={Boolean(appleError)} autoComplete="off" type="password" value={appPassword} onChange={(event) => setAppPassword(event.target.value)} placeholder="xxxx-xxxx-xxxx-xxxx" required /></label></div>{appleError && <div className={styles.appleError} role="alert"><strong>Apple Calendar konnte nicht verbunden werden</strong><span>{appleError}</span><small>Prüft Apple-ID und App-spezifisches Passwort. Erstellt bei Bedarf in eurem Apple-Account ein neues Passwort für „Ouivio Kalender“.</small></div>}<button disabled={testing}>{testing ? "Apple-Zugang wird geprüft …" : "Apple-Zugang sicher prüfen"}</button><small>Der Test überträgt die Angaben verschlüsselt zum Server und verwirft das Passwort direkt danach.</small></form>}
