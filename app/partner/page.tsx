...
function ServiceAreaSettings({demoMode,items,partnerId,onChange}:{demoMode:boolean;items:PartnerServiceArea[];partnerId:string;onChange:(items:PartnerServiceArea[])=>void}){const labels={location:"Location",catering:"Catering",photography:"Fotografie"} as const;const [note,setNote]=useState("");const save=async(area:PartnerServiceArea)=>{const next={...area,externalAddonsNote:note||area.externalAddonsNote};if(demoMode){onChange(items.map(x=>x.serviceType===area.serviceType?next:x));return;}const saved=await savePartnerServiceArea(partnerId,{serviceType:next.serviceType,isActive:next.isActive,bookingMode:next.bookingMode,externalAddonsPolicy:next.externalAddonsPolicy,externalAddonsNote:next.externalAddonsNote});onChange(items.map(x=>x.serviceType===saved.serviceType?saved:x));};const all=(['location','catering','photography'] as const).map(type=>items.find(x=>x.serviceType===type)||{id:type,partnerId,serviceType:type,isActive:false,bookingMode:'standalone' as const,externalAddonsPolicy:'allowed' as const,externalAddonsNote:''});return <section className={styles.specialized}><small>Partnerprofil</small><h1>Eure Angebotsbereiche</h1><p>Ihr entscheidet für jede Leistung, ob sie einzeln, nur als Ergänzung, im Bundle oder als Komplettpaket buchbar ist.</p><div className={styles.categoryGrid}>{all.map(area=><article key={area.serviceType}><strong>{labels[area.serviceType]}</strong><label><input checked={area.isActive} onChange={e=>void save({...area,isActive:e.target.checked})} type="checkbox"/> Aktiv anbieten</label><label>Buchungsart<select value={area.bookingMode} onChange={e=>void save({...area,bookingMode:e.target.value as PartnerServiceArea['bookingMode']})}><option value="standalone">Einzeln buchbar</option><option value="add_on">Nur als Add-on</option><option value="bundle_only">Nur im Bundle</option><option value="full_package">Komplettpaket</option></select></label><label>Externe Ergänzungen<select value={area.externalAddonsPolicy} onChange={e=>void save({...area,externalAddonsPolicy:e.target.value as PartnerServiceArea['externalAddonsPolicy']})}><option value="allowed">Erlaubt</option><option value="restricted">Eingeschränkt</option><option value="not_allowed">Nicht erlaubt</option></select></label><label>Hinweis<textarea defaultValue={area.externalAddonsNote} onChange={e=>setNote(e.target.value)} rows={2}/></label></article>)}</div></section>}
