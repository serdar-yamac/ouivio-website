...
export default function DiscoverDemo() {
  ...
  return <main className={styles.shell}>
    ...
    {selected&&<section className={styles.detail} aria-label={`${selected.name} Profil`}><button className={styles.close} onClick={()=>setSelected(null)}>Schließen ×</button><div className={styles.detailImage}><Image alt={`${selected.name} Titelbild`} fill sizes="50vw" src={selected.image}/></div><div className={styles.detailBody}><small>{selected.category} · {selected.match} Match</small><h2>{selected.name}</h2><p>{selected.summary}</p><h3>{selected.category==='Catering'?'Beispielgerichte':selected.category==='Fotografie'?'Pakete & Leistungen':'Bereiche & Möglichkeiten'}</h3><ul>{selected.offerings.map(item=><li key={item}>{item}</li>)}</ul><h3>Auf einen Blick</h3><ul>{selected.facts.map(item=><li key={item}>{item}</li>)}</ul>{'profileHref' in selected&&<Link className={styles.fullProfile} href={selected.profileHref}>Vollständiges Portfolio ansehen →</Link>}<button className={styles.request}>Verfügbarkeit anfragen</button><em>Demo – noch keine echte Anfrage</em></div></section>}
  </main>;
}
