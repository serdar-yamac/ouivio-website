...
export default function DiscoverDemo() {
  ...
  return <main className={styles.shell}>
    <header><Link href="/">Ouivio<span>.</span></Link><div><small>Kundendemo</small><strong>Anbieter entdecken</strong></div><Link className={styles.partnerLink} href="/discover/cart?demo=1">Warenkorb</Link><Link className={styles.partnerLink} href="/partner?demo=1">Partneransicht</Link></header>
    ...
    {selected&&<section className={styles.detail} aria-label={`${selected.name} Profil`}><button className={styles.close} onClick={()=>setSelected(null)}>Schließen ×</button><div className={styles.detailImage}><Image alt={`${selected.name} Titelbild`} fill sizes="50vw" src={selected.image}/></div><div className={styles.detailBody}><small>{selected.category} · {selected.match} Match</small><h2>{selected.name}</h2><p>{selected.summary}</p><h3>{selected.category==='Catering'?'Beispielgerichte':selected.category==='Fotografie'?'Pakete & Leistungen':'Bereiche & Möglichkeiten'}</h3><ul>{selected.offerings.map(item=><li key={item}>{item}</li>)}</ul><h3>Auf einen Blick</h3><ul>{selected.facts.map(item=><li key={item}>{item}</li>)}</ul>{'profileHref' in selected&&<Link className={styles.fullProfile} href={selected.profileHref}>Vollständiges Portfolio ansehen →</Link>}<button className={styles.request}>Verfügbarkeit prüfen</button><em>Sofortige Prüfung · Buchung ohne Anfrage</em></div></section>}
  </main>;
}
